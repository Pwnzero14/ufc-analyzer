...
import { AllLines } from './types/index.js';
...
function render(lines: AllLines): void {
...
document.getElementById('auto-fetch-btn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    const count = Object.values(result?.results || {}).reduce(
...
document.getElementById('retry-pick6-btn')?.addEventListener('click', async () => {
  const btn = document.getElementById('retry-pick6-btn') as HTMLButtonElement;
...
document.getElementById('fetch-dk-tds-btn')?.addEventListener('click', async () => {
  const btn = document.getElementById('fetch-dk-tds-btn') as HTMLButtonElement;
...
(globalThis as any).ufc_popup = { loadAndRender };
