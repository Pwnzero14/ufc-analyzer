...
export class PropLinePredictorService {
  ...
  static computeBookPrior(
    ...
    for (const r of archive) {
      ...
      if (books && (!r.platform || !books.has(r.platform))) continue;
      if (!propTypes.includes(String(r.propType))) continue;
   ...
   */
  static applyBookPrior(
    sp: StatPrediction,
    prior: { median: number; sampleCount: number } | null,
    label: string,
  ): StatPrediction {
    if (!sp || !prior || prior.sampleCount < 5 || !Number.isFinite(sp.line)) return sp;
    const w = clamp(0.10 + prior.sampleCount * 0.02, 0.20, 0.35);
    const before = sp.line;
    const blended = round1((1 - w) * before + w * prior.median);
    if (blended === before) return sp;
    return {
      ...sp,
      line: blended,
  ...
  static applyMarketAnchorFor(
    ...
    if (!sp || postedLine == null || !Number.isFinite(postedLine) || postedLine <= 0) return sp;
    if (!Number.isFinite(sp.line) || !Number.isFinite(shift)) return sp;
    const fair = postedLine + shift;
    const cap = Math.max(minCap, this.ANCHOR_GAP_FRACTION * fair);
    const gap = sp.line - fair;
    if (Math.abs(gap) <= cap) return sp;
  ...
  static readonly DEBUT_ML_GAP: Record<string, number> = { ss: 13.4, ss_r1: 4.7, fp: 16.0 };

  static applyDebutMoneylineSplit(
    pred: PropPrediction,
    moneyline: number | null | undefined,
    hasHistory: boolean,
  ): PropPrediction {
    const ml = Number(moneyline);
    if (hasHistory || !Number.isFinite(ml) || ml === 0) return pred;
    const side = ml < 0 ? 1 : -1; // negative American price = favourite
    const role = ml < 0 ? 'favourite' : 'underdog';
    const bump = (sp: StatPrediction | undefined, key: string, floor: number): StatPrediction | undefined => {
      if (!sp || !Number.isFinite(sp.line)) return sp;
      const half = (this.DEBUT_ML_GAP[key] ?? 0) / 2;
      if (!half) return sp;
      const before = sp.line;
      const after = round1(Math.max(floor, before + side * half));
      if (after === before) return sp;
      return {
        ...sp,
        line: after,
  ...
  static bookCalibration(
    ...
    for (const [book, byStat] of Object.entries(bt.byBook)) {
      for (const [stat, cell] of Object.entries(byStat)) {
        if (!cell || cell.n < MIN_OFFSET_N || !Number.isFinite(cell.bias)) continue;
    ...
    for (const [stat, cell] of Object.entries(bt.byStat)) {
      if (cell && cell.n >= MIN_OFFSET_N && Number.isFinite(cell.bias)) global[stat] = cell.bias;
    }
  ...
  static calibrateToBooks(pred: PropPrediction, cal: BookCalibration): PropPrediction {
    const apply = (sp: StatPrediction, statLabel: string): StatPrediction => {
      if (!sp || !Number.isFinite(sp.line)) return sp;
      const off = cal.global[statLabel] ?? 0;
      if (!off && !Number.isFinite(off)) return sp;
      const before = sp.line;
      const snapped = this.snapToGrid(before - off, 0.5);
      if (snapped === before) return sp;
      return {
        ...sp,
        line: snapped,
        reasons: [
