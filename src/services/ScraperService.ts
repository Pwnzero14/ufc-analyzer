...
export class ScraperService {
  ...
  static scrapePick6(): Fighter[] {
    ...
    try {
      ...
      document.querySelectorAll(config.cardButton).forEach((btn) => {
        ...
        const tightCard = btn.closest('div[class]') as HTMLElement | null;
        let cardEl: HTMLElement | null = tightCard;
        let probe: HTMLElement | null = tightCard;
        for (let i = 0; i < 6 && probe; i++) {
          const t = probe.innerText || '';
          if ((t.match(/\bvs\b/gi) || []).length > 1) break;
          cardEl = probe;
          if (/\bMore\b/i.test(t)) break;
          probe = (probe.parentElement?.closest('div[class]') as HTMLElement | null) || null;
        }
      ...
      if (Object.keys(fighters).length === 0) {
        document.querySelectorAll(config.playerCard).forEach((card) => {
          ...
          if (fpMatch) { fighters[name].line_fp = parseFloat(fpMatch[1]); fighters[name].fp_under_available = lessBtn; }
          if (ssMatch) { fighters[name].line_ss = parseFloat(ssMatch[1]); fighters[name].ss_under_available = lessBtn; }
          if (tdMatch) { fighters[name].line_td = parseFloat(tdMatch[1]); fighters[name].td_under_available = lessBtn; }
          if (ctrlMatchMMSS) {
  ...
  static scrapeDKSportsbookProps(): Fighter[] {
    ...
    try {
          ...
          for (let i = 0; i < 15; i++) {
            ...
            if (lineOddsMatch) {
              ...
              if (!isNaN(line) && line > 0 && line < 200) {
                ...
                fighters[name].line_ss = line;
                if (odds) fighters[name].ss_over_odds = odds;
          ...
          for (let i = 0; i < 15; i++) {
            ...
            if (lineOddsMatch) {
              ...
              if (!isNaN(line) && line >= 0 && line < 20) {
                ...
                fighters[name].line_td = line;
                if (odds) fighters[name].td_over_odds = odds;
      ...
      if (Object.keys(fighters).length === 0) {
        ...
        while ((match = ssRegex.exec(pageText)) !== null) {
          ...
          if (!isNaN(line) && line > 0 && line < 200) {
            ...
            fighters[name].line_ss = line;
            if (overOdds) fighters[name].ss_over_odds = overOdds;
            if (underOdds) fighters[name].ss_under_odds = underOdds;
          }
        }
        ...
        while ((match = tdRegex.exec(pageText)) !== null) {
          ...
          if (!isNaN(line) && line >= 0 && line < 20) {
            ...
            fighters[name].line_td = line;
            if (overOdds) fighters[name].td_over_odds = overOdds;
            if (underOdds) fighters[name].td_under_odds = underOdds;
          }
        }
      }
      ...
      const result = Object.values(fighters).filter(
        (f) => f.line_ss !== null || f.line_td !== null
      );

      this.log(
        'draftkings_sportsbook',
        `Found ${result.length} fighters with props. Samples: ${result
          .slice(0, 3)
          .map(
            (f) =>
              `${f.name} (SS: ${f.line_ss || '—'} @${f.ss_over_odds || '—'}, TD: ${f.line_td || '—'} @${f.td_over_odds || '—'})`
          )
          .join('; ')}`
      );
