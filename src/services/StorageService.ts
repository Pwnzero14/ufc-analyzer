...
export class StorageService {
  ...
  // ── LINES (Pick6, Underdog, Betr) ──────────────────────────────────────

  static async getLines(platform?: 'pick6' | 'underdog' | 'betr'): Promise<AllLines> {
    try {
      const keys = platform
        ? [`lines_${platform}`]
        : ['lines_pick6', 'lines_underdog', 'lines_betr'];
  ...
  static async clearLines(): Promise<void> {
    try {
      await this.chromeClear(['lines_pick6', 'lines_underdog', 'lines_betr']);
