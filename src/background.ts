  ...
  if (Number.isFinite(seedEventMs) && Date.now() > seedEventMs) {
    try {
      ...
      await new Promise<void>((res) =>
        chrome.storage.local.remove(['betr_seed_hash', 'betr_event_date'], () => res())
      );
      const persisted = await new Promise<Record<string, any>>((res) =>
        chrome.storage.local.get(['lines_betr', 'lines_betr_manual_v1'], res)
      );
  ...
  if (request.type === 'LINES_CAPTURED') {
    ...
    if (pg && /^\d+$/.test(pg)) {
      ...
      chrome.storage.local.get(['pick6_active_pick_group', 'upcoming_ufc_card'], (res) => {
        ...
        const card = typeof cardRaw === 'string' ? (() => { try { return JSON.parse(cardRaw); } catch { return null; } })() : cardRaw;
...
async function fetchFightDetails(url: string): Promise<Array<{
  ...
  fightTimeMins: number; ssBody: number; ssLeg: number;
}>> {
  try {
...
async function fetchAndSettleFromUFCStats(opts?: { forceEventName?: string; includeZeroResults?: boolean; recentOnly?: boolean }): Promise<{ settled: number; skipped: number; errors: string[]; purged?: number }> {
...
async function _fetchAndSettleFromUFCStats(opts?: { forceEventName?: string; includeZeroResults?: boolean; recentOnly?: boolean }): Promise<{ settled: number; skipped: number; errors: string[]; purged?: number }> {
  ...
  try {
    ...
    const _surname = (s: string) => _baseNorm(String(s || '')).split(' ').filter(Boolean).pop() || '';
    const eventRosterSurnames = new Map<string, Set<string>>(); // normEvent -> surnames present on the card
    ...
    const createdRows: PropArchiveRecord[] = [];

    function ensureResultRow(name: string, opponent: string, event: string, propType: string, result: number, date: string): number {
      ...
      for (const row of archive) {
        if (_normName(String(row.fighter || "")) !== nName) continue;
      ...
      const row = {
        ...
        event,
        date,
        propType: propType as PropArchiveRecord["propType"],
        result,
      } as PropArchiveRecord;
      archive.push(row);
    ...
    for (const archiveEvent of eventNames) {
      ...
      // Record this event's actual roster (by surname) so foreign unresolved rows can be purged.
      {
        ...
        for (const f of allFightResults) { const s = _surname(f.name); if (s) surnames.add(s); }
        if (surnames.size) eventRosterSurnames.set(_normEvent(archiveEvent), surnames);
      }
      ...
      for (const f of allFightResults) {
        ...
        for (const [prop, value] of STATS) {
          ...
          if (filled === 0) created += ensureResultRow(f.name, oppByName.get(f.name) || "", archiveEvent, prop, value, match.date);
        }
        if (n > 0 || created > 0) {
    ...
    if (unmatchedEvents.length > 0) {
      ...
      for (const archiveEvent of unmatchedEvents) {
        ...
        const fbSurnames = new Set<string>();
        for (const f of matchedEntry.results) {
          if (!f.name) continue;
          const last = f.name.trim().split(/\s+/).pop()!.toLowerCase();
          cardLastNameMap.set(last, f);
          const s = _surname(f.name); if (s) fbSurnames.add(s);
        }
        if (fbSurnames.size) eventRosterSurnames.set(_normEvent(archiveEvent), fbSurnames);
    ...
    if (settled > 0 || eventRosterSurnames.size > 0) {
      ...
      await PropArchiveService.mutate((freshArchive) => {
        ...
        if (createdRows.length) {
          const seen = new Set(kept.map((r: any) =>
            `${_normName(String(r.fighter || ''))}|${_normEvent(String(r.event || ''))}|${_normProp(String(r.propType || ''))}`));
          let added = 0;
          ...
          if (added) console.log(`[UFC Settle] added ${added} result-only row(s) at write time`);
        }
        keptLen = kept.length;
        return kept;
      });
...
async function archivePlatformPropLines(
  ...
  const cardNames = new Set<string>();
  const cardSurnames = new Set<string>();
  const surnameOf = (n: string) => String(n || '').trim().toLowerCase().replace(/[^a-z\s']/g, '').split(/\s+/).filter(Boolean).pop() || '';
  for (const bout of card.fighters || []) {
    for (const nm of [bout?.f1, bout?.f2]) {
      ...
      if (last.length >= 3) cardSurnames.add(last);
    }
  }
  const onCard = (n: any): boolean => {
    const norm = String(n || '').trim().toLowerCase();
    if (!norm) return false;
    if (cardNames.has(norm)) return true;
    const last = surnameOf(norm);
  ...
  for (const f of fighters) {
    ...
    if (cardNames.size > 0 && !onCard(fighter) && !onCard(opponent)) continue;
...
async function eventHasSettledRows(event: string): Promise<boolean> {
  try {
    const norm = (s: string) => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const target = norm(event);
    if (!target) return false;
    const payload = await new Promise<Record<string, any>>((resolve) =>
      chrome.storage.local.get(['prop_archive_v1'], (r) => resolve(r || {})));
    const rows: any[] = Array.isArray(payload.prop_archive_v1) ? payload.prop_archive_v1 : [];
...
function mergeFighters(
  ...
  const push = (fighter: any) => {
    ...
    if (!map.has(key)) {
      ...
      if (fighter.line_kd != null) merged.line_kd = fighter.line_kd;
      if (fighter.kd_under_available != null) merged.kd_under_available = fighter.kd_under_available;
      ...
      if (fighter.betr_fp_over_avail != null) merged.betr_fp_over_avail = fighter.betr_fp_over_avail;
      if (fighter.betr_fp_under_avail != null) merged.betr_fp_under_avail = fighter.betr_fp_under_avail;
      if (fighter.betr_ss_over_avail != null) merged.betr_ss_over_avail = fighter.betr_ss_over_avail;
      if (fighter.betr_ss_under_avail != null) merged.betr_ss_under_avail = fighter.betr_ss_under_avail;
      if (fighter.betr_td_over_avail != null) merged.betr_td_over_avail = fighter.betr_td_over_avail;
      if (fighter.betr_td_under_avail != null) merged.betr_td_under_avail = fighter.betr_td_under_avail;
      if (fighter.betr_ft_over_avail != null) merged.betr_ft_over_avail = fighter.betr_ft_over_avail;
      if (fighter.betr_ft_under_avail != null) merged.betr_ft_under_avail = fighter.betr_ft_under_avail;
 ...
 */
let _linesCaptureChain: Promise<void> = Promise.resolve();

function handleLinesCaptured(platform: string, data: any): Promise<void> {
  _linesCaptureChain = _linesCaptureChain
    .catch(() => { /* a failed capture must not break the chain for later ones */ })
    .then(() => handleLinesCapturedInner(platform, data));
  return _linesCaptureChain;
}

async function handleLinesCapturedInner(platform: string, data: any): Promise<void> {
  try {
...
function parseUnderdogApiFighters(data: any): Array<{ name: string; line_fp: number | null; line_ss: number | null; line_ss_r1: number | null; line_ss_body: number | null; line_ss_leg: number | null; line_td: number | null; line_ft: number | null; opponent: string | null; ss_over_odds: number | null; ss_under_odds: number | null; td_over_odds: number | null; td_under_odds: number | null; ft_over_odds: number | null; ft_under_odds: number | null; ud_ss_over_avail: boolean | null; ud_ss_under_avail: boolean | null; ud_td_over_avail: boolean | null; ud_td_under_avail: boolean | null; ud_ft_over_avail: boolean | null; ud_ft_under_avail: boolean | null }> {
  ...
  for (const line of lines as any[]) {
    ...
    else if (title.includes('takedown') && !title.includes('def') && !title.includes('attempt')) lineType = 'td';
...
function parsePrizePicksApiFighters(data: any): Array<{ name: string; line_fp: number | null; line_ss: number | null; line_ss_r1: number | null; line_ss_body: number | null; line_ss_leg: number | null; line_td: number | null; line_ft: number | null; line_kd: number | null; kd_under_available: boolean | null; opponent: string | null }> {
  const fighters: Record<string, { name: string; line_fp: number | null; line_ss: number | null; line_ss_r1: number | null; line_ss_body: number | null; line_ss_leg: number | null; line_td: number | null; line_ft: number | null; line_kd: number | null; kd_under_available: boolean | null; opponent: string | null }> = {};
  ...
  const upsert = (name: string, type: 'fp'|'ss'|'ss_r1'|'ss_body'|'ss_leg'|'td'|'ft'|'kd', value: number, opponent: string | null = null) => {
    if (!fighters[name]) fighters[name] = { name, line_fp: null, line_ss: null, line_ss_r1: null, line_ss_body: null, line_ss_leg: null, line_td: null, line_ft: null, line_kd: null, kd_under_available: null, opponent };
  ...
  for (const p of projections) {
    ...
    let lineType: 'fp'|'ss'|'ss_r1'|'ss_body'|'ss_leg'|'td'|'ft'|'kd'|null = null;
    ...
    else if (stat.includes('takedown') && !stat.includes('attempt')) lineType = 'td';
    ...
    if (lineType === 'kd' && line >= 5) continue;
 ...
 * nothing is cleared.
 */
const RECONCILABLE_LINE_FIELDS: Array<[string, string[]]> = [
...
function hasAnyReconcilableLine(f: any): boolean {
...
function reconcileRemovals(merged: Array<any>, incoming: Array<any>, platform: string): number {
  ...
  for (const f of incoming || []) {
    ...
    if (!prev) { freshByName.set(k, { ...f }); continue; }
    for (const [field] of RECONCILABLE_LINE_FIELDS) {
      if (prev[field] == null && f[field] != null) prev[field] = f[field];
    }
  }
  ...
  for (const f of merged || []) {
    ...
    for (const [field, companions] of RECONCILABLE_LINE_FIELDS) {
      ...
      for (const c of companions) f[c] = null;
      cleared++;
    }
  }
  if (cleared) {
...
function parseBetrGraphQLFighters(data: any): Array<any> {
  ...
  for (const ev of events) {
    ...
    for (const team of ev?.teams || []) {
      for (const p of team?.players || []) {
        ...
        for (const pr of p?.projections || []) {
          if (pr?.marketStatus && pr.marketStatus !== 'OPENED') continue;
          const outcomes = (pr?.allowedOptions || []).map((o: any) => String(o?.outcome || '').toUpperCase());
          ...
          switch (String(pr?.key || '').toUpperCase()) {
            case 'FANTASY_POINTS':
              rec.line_fp = v; rec.betr_fp_over_avail = over; rec.betr_fp_under_avail = under; break;
            case 'SIG_STRIKES':
              rec.line_ss = v; rec.betr_ss_over_avail = over; rec.betr_ss_under_avail = under; break;
            case 'TAKEDOWNS':
              rec.line_td = v; rec.betr_td_over_avail = over; rec.betr_td_under_avail = under; break;
            case 'FIGHT_TIME':
              rec.line_ft = v; rec.betr_ft_over_avail = over; rec.betr_ft_under_avail = under; break;
    ...
    const names: string[] = [];
    for (const team of ev?.teams || []) for (const p of team?.players || []) {
      const n = `${p?.firstName || ''} ${p?.lastName || ''}`.trim();
      if (n) names.push(n);
    }
    if (names.length === 2) {
      if (out[names[0]]) out[names[0]].opponent = names[1];
      if (out[names[1]]) out[names[1]].opponent = names[0];
    }
  }
  return Object.values(out).filter((f: any) =>
    f.line_fp != null || f.line_ss != null || f.line_td != null || f.line_ft != null);
}

async function fetchBetrFromBackground(): Promise<UnderdogCoverage> {
  ...
  if (!fresh.length) return getUnderdogStatCoverage(mergedFighters);
...
const DK_ROUND_START_URL =
  'https://sportsbook-nash.draftkings.com/api/sportscontent/dkusoh/v1/leagues/9034/categories/677/subcategories/5800';

async function refreshDKRoundStartFromApi(reason: string): Promise<number> {
  try {
    const res = await fetch(DK_ROUND_START_URL, {
      signal: AbortSignal.timeout(15000),
      headers: { accept: 'application/json' },
    });
    if (!res.ok) { console.warn(`[UFC Odds] DK round-start HTTP ${res.status} (${reason})`); return 0; }
    const data: any = await res.json();
    const markets = Array.isArray(data?.markets) ? data.markets : [];
    const selections = Array.isArray(data?.selections) ? data.selections : [];
    ...
    const marketInfo: Record<string, { round: number; eventId: string }> = {};
    ...
    const out: Record<string, Record<string, { yes: number | null; no: number | null }>> = {};
...
async function refreshDKFighterPropsFromApi(reason: string): Promise<number> {
  ...
  const fetchStat = async (
    ...
    try {
      ...
      if (!res.ok) { console.warn(`[UFC Odds] DK ${kind} O/U HTTP ${res.status} (${reason})`); return null; }
      const data: any = await res.json();
      const markets = Array.isArray(data?.markets) ? data.markets : [];
      const selections = Array.isArray(data?.selections) ? data.selections : [];
      const events = Array.isArray(data?.events) ? data.events : [];
      if (!markets.length || !selections.length) return null;

      // eventId → [rawName, rawName] (for opponent pairing)
      const eventParts: Record<string, string[]> = {};
      for (const ev of events) {
        const names = (Array.isArray(ev?.participants) ? ev.participants : [])
          .map((p: any) => String(p?.name || '').trim()).filter(Boolean);
        if (ev?.id && names.length >= 2) eventParts[String(ev.id)] = names;
      }
      ...
      const legs: Record<string, { line: number | null; over: number | null; under: number | null }> = {};
      for (const sel of selections) {
        const mid = String(sel?.marketId || '');
        if (!mkt[mid]) continue;
        ...
        if (!legs[mid]) legs[mid] = { line: null, over: null, under: null };
        if (Number.isFinite(line)) legs[mid].line = line;
        if (side.startsWith('over')) legs[mid].over = american;
        else if (side.startsWith('under')) legs[mid].under = american;
      }

      const byName: Record<string, { line: number; over: number | null; under: number | null; eventId: string }> = {};
      for (const [mid, info] of Object.entries(mkt)) {
        const l = legs[mid];
        if (!l || l.line == null || !inBounds(l.line)) continue;
  ...
  try {
    ...
    for (const [name, v] of Object.entries(ss?.byName || {})) {
      const f = ensure(name, v.eventId); f.line_ss = v.line; f.ss_over_odds = v.over; f.ss_under_odds = v.under;
    }
      ...
      if (platform === 'pick6') {
        try {
          const cached = await new Promise<Record<string, any>>((res) =>
            chrome.storage.local.get(['pick6_active_pick_group', 'pick6_active_url', 'pick6_pick_group_event', 'upcoming_ufc_card'], (r) => res(r || {}))
          );
...
async function fetchUpcomingUFCCard(forceRefresh = false): Promise<UpcomingCardCache | null> {
  ...
  try {
    ...
    if (headlinerMatch) {
      const surname = (s: string): string => s.trim().toLowerCase().split(/\s+/).pop() || '';
      const h1 = surname(headlinerMatch[1]);
      const h2 = surname(headlinerMatch[2]);
      const nameHas = (full: string, sn: string): boolean => {
        if (!sn) return false;
        const parts = full.trim().toLowerCase().split(/\s+/);
        return parts[parts.length - 1] === sn || parts.includes(sn);
      };
      const mainEvent = fighters.find((f) =>
        (nameHas(f.f1, h1) || nameHas(f.f2, h1)) && (nameHas(f.f1, h2) || nameHas(f.f2, h2))
      );
      if (mainEvent) mainEvent.scheduledRounds = 5;
    }
