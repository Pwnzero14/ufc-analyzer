// Venue factors: altitude (m) and cage size (ft) per UFC host city/arena.
// Read by lean engine (altitude → cardio/SS penalty) and FP projection
// (small cage → grappler control bonus). Climate notes are informational.
...
export const VENUE_DB: Record<string, VenueFactorEntry> = {
...
export const DEFAULT_VENUE: VenueFactorEntry = { altitudeMeters: 0, cageSizeFt: 30 };

export function resolveVenueFactor(location: string | undefined): { factor: VenueFactorEntry; label: string } {
