export function detectStreak(history) {
    if (!history?.length)
    ...
    if (winStreak >= 3)
        return { type: 'hot', count: winStreak, text: `${winStreak}-fight win streak` };
    if (lossStreak >= 2)
        return { type: 'cold', count: lossStreak, text: `${lossStreak}-fight losing streak` };
    ...
    if (fpFights.length >= 3) {
        ...
        if (rising >= 2)
            return { type: 'hot', count: rising, text: 'FP trending up last 3 fights' };
        if (falling >= 2)
            return { type: 'cold', count: falling, text: 'FP trending down last 3 fights' };
    }
    return { type: 'neutral', count: 0, text: '' };
}
export function calcWeightedAvgFP(history) {
...
export function calcFPStats(history) {
    const fps = history.filter(f => f.fp != null && f.fp > 0).map(f => f.fp);
    if (!fps.length)
        return { floor: null, ceiling: null, stdDev: null, consistency: null, median: null };
    if (fps.length === 1)
        return { floor: parseFloat(fps[0].toFixed(1)), ceiling: parseFloat(fps[0].toFixed(1)), stdDev: 0, consistency: 100, median: parseFloat(fps[0].toFixed(1)) };
...
export function calcPerRoundFP(history) {
