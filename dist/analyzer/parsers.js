export function parseCareerStats(html) {
    const stats = {};
    const li = (label) => {
...
export function parseFightHistoryLinks(html) {
    const fights = [];
    ...
    for (const rowM of html.matchAll(/<tr[^>]*>([\s\S]*?)<\/tr>/gi)) {
        ...
        for (const t of tds) {
            if (t.includes(':'))
                continue;
            const n = parseInt(t);
            if (!isNaN(n) && n >= 1 && n <= 5 && t.trim().length <= 2) {
                round = n;
                break;
            }
        }
...
export function parseFightDetailStats(html, fighterName, fighterDetailUrl) {
    const clean = (s) => (s || '').replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    if (methodM) {
        ...
        if (raw.includes('ko') || raw.includes('tko'))
            detailMethod = 'KO/TKO';
        else if (raw.includes('sub'))
            detailMethod = 'SUB';
        else if (raw.includes('unanimous'))
            detailMethod = 'U-DEC';
        else if (raw.includes('split'))
            detailMethod = 'S-DEC';
        else if (raw.includes('majority'))
            detailMethod = 'M-DEC';
        else if (raw.includes('decision'))
            detailMethod = 'DEC';
        else if (raw.includes('no contest'))
            detailMethod = 'NC';
        else if (raw.includes('disq'))
            detailMethod = 'DQ';
    }
    const roundM = html.match(/Round:\s*<\/i>\s*(?:<[^>]+>\s*)*(\d+)/i)
        || html.match(/Round:\s*(\d+)/i);
    if (roundM)
        detailRound = parseInt(roundM[1]);
    let detailTimeSecs = null;
    ...
    if (timeM) {
        ...
        if (detailRound && detailRound >= 1)
            detailTimeSecs = ((detailRound - 1) * 5 * 60) + roundClockSecs;
        else
            detailTimeSecs = roundClockSecs;
    }
    ...
    if (!totalsTable)
        return { method: detailMethod, round: detailRound, timeSecs: detailTimeSecs };
    ...
    if (fIdx === 0 && tds[0]) {
        ...
        if (tds[0][1] && nameParts.every(p => tds[0][1].toLowerCase().includes(p)))
            fIdx = 1;
    }
    ...
    if (ctrlM)
        ctrlSecs = parseInt(ctrlM[1]) * 60 + parseInt(ctrlM[2]);
    ...
    if (perRoundTable) {
        ...
        if (prRows.length > 0) {
            ...
            const r1Val = (colIdx) => r1Tds[colIdx]?.[fIdx] || r1Tds[colIdx]?.[0] || '';
...
export function parseFightDetailStatsOpponent(html, fighterName, fighterDetailUrl) {
    ...
    if (perRoundTable) {
        ...
        if (prRows.length > 0) {
            ...
            const r1Val = (colIdx) => r1Tds[colIdx]?.[oppIdx] || r1Tds[colIdx]?.[0] || '';
            sigStrR1 = firstNum(r1Val(2));
        }
    }
