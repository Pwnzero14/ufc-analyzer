export function styleMatchupEdge(styleA, styleB, dbA, dbB) {
    const edges = [];
    let delta = 0;
...
export function calcOpponentDefenseScore(oppDB, _line) {
    if (!oppDB?.loaded || !oppDB.history?.length)
        return { delta: 0, edges: [] };
    const edges = [];
    let delta = 0;
    if (oppDB.sapm != null) {
        if (oppDB.sapm < 3.0) {
...
export function calcMatchupPatternEdge(db, oppDB, ssLine, tdLine, fpLine, statsCache) {
    if (!db?.loaded || !oppDB?.loaded || !db.history?.length)
        return { score: 0, ssScore: 0, tdScore: 0, reasons: [] };
    ...
    if (oppStyle) {
        ...
        if (styleMatches.length >= 2) {
            const avgSS_vsStyle = styleMatches.filter((h) => h.sigStr != null).reduce((s, h) => s + (h.sigStr || 0), 0) / styleMatches.length;
            const avgTD_vsStyle = styleMatches.filter((h) => h.td != null).reduce((s, h) => s + (h.td || 0), 0) / styleMatches.length;
            const avgFP_vsStyle = styleMatches.filter((h) => h.fp != null).reduce((s, h) => s + (h.fp || 0), 0) / styleMatches.length;
            ...
            if (fpLine) {
                ...
                if (fpDiff > 8) {
                    score += 1;
                    reasons.push({ icon: 'pos', text: `Avg ${avgFP_vsStyle.toFixed(1)} FP ${label} — ${styleMatches.filter(h => (h.fp || 0) > fpLine).length}/${styleMatches.length} over FP line` });
                }
                else if (fpDiff < -8) {
                    score -= 1;
                    reasons.push({ icon: 'neg', text: `Avg ${avgFP_vsStyle.toFixed(1)} FP ${label} — below FP line historically` });
                }
            }
        }
    }
    if (oppStance) {
        ...
        if (stanceMatches.length >= 2) {
            ...
            if (ssLine) {
                ...
                if (ssDiff > 12) {
                    ssScore += 1.2;
                    reasons.push({ icon: 'pos', text: `Avg ${avgSS_vsStance.toFixed(0)} SS ${label}` });
                }
                else if (ssDiff < -12) {
                    ssScore -= 1.2;
                    reasons.push({ icon: 'neg', text: `Only ${avgSS_vsStance.toFixed(0)} avg SS ${label} — stance creates problems` });
                }
            }
            if (tdLine) {
                ...
                if (tdDiff > 1) {
                    tdScore += 0.8;
                    reasons.push({ icon: 'pos', text: `Avg ${avgTD_vsStance.toFixed(1)} TDs ${label}` });
                }
                else if (tdDiff < -1) {
                    tdScore -= 0.8;
                    reasons.push({ icon: 'neg', text: `Only ${avgTD_vsStance.toFixed(1)} avg TDs ${label}` });
                }
            }
        }
    }
    if (oppStrDef != null) {
        ...
        const tierMatches = history.filter(h => {
            const pastOppDB = statsCache[h.opp];
            if (!pastOppDB?.loaded || pastOppDB.strDef == null)
                return false;
    ...
    if (oppTdDef != null) {
        ...
        const tdTierMatches = history.filter(h => {
            const pastOppDB = statsCache[h.opp];
            if (!pastOppDB?.loaded || pastOppDB.tdDef == null)
                return false;
...
//# sourceMappingURL=style-matchup.js.map