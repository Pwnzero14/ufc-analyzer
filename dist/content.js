    ...
    try {
        ...
        document.querySelectorAll('[data-testid="cardButton"]').forEach((btn) => {
            ...
            if (ctrlMMSS) {
                ...
                if (!isNaN(mm) && !isNaN(ss))
                    ctrlLine = parseFloat((mm + ss / 60).toFixed(2));
            }
            else if (ctrlDec) {
        ...
        if (Object.keys(fighters).length === 0) {
            ...
            const findCardText = (name) => {
                if (name in cardTextByName)
                    return cardTextByName[name];
                let found = null;
                ...
                for (const el of leaves) {
                    ...
                    for (let i = 0; i < 12 && c.parentElement; i++) {
                        ...
                        const t = c.innerText || '';
                        if (t.length > 1500)
                            break; // grew past a single card — try the next leaf
                        if (/\bMore\b/i.test(t) || /\bLess\b/i.test(t)) {
                            found = t;
                            break;
                        }
                    }
            ...
            for (const m of lineBlocks) {
                ...
                if (!name || name.length > 45 || isNaN(val))
                    continue;
                if (!fighters[name])
                    fighters[name] = { name, line_fp: null, line_ss: null, line_td: null, opponent: null };
                ...
                if (cardText != null) {
                    ...
                    if (isFp)
                        fighters[name].fp_under_available = less;
                    else if (stat.includes('significant'))
                        fighters[name].ss_under_available = less;
                    else if (stat.includes('takedown'))
                        fighters[name].td_under_available = less;
                }
            }
        }
    ...
    try {
        ...
        const mergeInto = (incoming) => {
            ...
            for (const f of incoming || []) {
                ...
                if (f.line_fp != null)
                    prev.line_fp = f.line_fp;
                if (f.line_ss != null)
                    prev.line_ss = f.line_ss;
                if (f.line_td != null)
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
            ...
            if (ctrlAttempted && hasEnoughCoverage(coverage))
                break;
...
function scrapePrizePicksCurrentView() {
    ...
    const upsert = (name, type, value, opponent = null) => {
        if (!name || !type || value == null || isNaN(value))
            return;
        if (!fighters[name])
            fighters[name] = { name, line_fp: null, line_ss: null, line_ss_r1: null, line_td: null, opponent };
    ...
    const parseFromText = (text) => {
        ...
        if (fpMatch)
            upsert(name, 'fp', parseFloat(fpMatch[1]), opponent);
        if (ssR1Match) {
        ...
        if (tdMatch)
            upsert(name, 'td', parseFloat(tdMatch[1]), opponent);
    };
    const cardSelectors = [
    ...
    if (Object.keys(fighters).length === 0) {
        ...
        blocks.forEach((b) => {
            if (!/Fantasy\s*(?:Points|Score)|Significant\s*Strikes?|Takedowns?/i.test(b))
                return;
            parseFromText(b);
        });
    }
...
function findButtonByText(labels) {
    ...
    const scored = candidates
        ...
        const score = (el.tagName === 'BUTTON' ? 4 : 0)
            + (role === 'button' ? 3 : 0)
            + (el.onclick ? 2 : 0)
...
function getPrizePicksCardCount() {
    const cardSelectors = [
        '[data-testid*="projection"]',
        '[class*="projection"]',
        '[class*="Projection"]',
...
async function waitForPrizePicksBoardReady(timeoutMs = 18000) {
...
async function clickButtonByLabels(context, labels, waitMs = 900) {
 ...
 * Control Time.
 */
async function clickButtonByLabelsWhenReady(context, labels, waitMs = 900, timeoutMs = 6000) {
...
async function clickPrizePicksButton(labels, waitMs = 900) {
        ...
        const mergeInto = (incoming) => {
            ...
            for (const f of incoming) {
                ...
                if (!map.has(key)) {
                    ...
                    if (f.line_ss != null)
                        prev.line_ss = f.line_ss;
                    if (f.line_ss_r1 != null)
                        prev.line_ss_r1 = f.line_ss_r1;
                    if (f.line_td != null)
                        prev.line_td = f.line_td;
                    if (!prev.opponent && f.opponent)
                        prev.opponent = f.opponent;
...
function scrapeDKBetHandle() {
    try {
        const results = [];
        const widgets = document.querySelectorAll('[data-testid="bet-breakdown"]');
        if (widgets.length === 0) {
            const fallback = [...document.querySelectorAll('*')].find((n) => (n.textContent || '').trim().toLowerCase() === '% of bets placed');
            if (fallback) {
                let box = fallback;
...
async function tryScrape(platform, scrapeFn) {
    ...
    const { maxAttempts, attemptIntervalMs, timeoutMs, stableTarget, minAttemptsBeforeResolve, scrollTimeoutMs, scrollIntervalMs, } = getScrapeProfile(platform);
    let bestResult = [];
    let stableCount = 0;
    let attempts = 0;
...
if (host.includes('pick6.draftkings.com')) {
    ...
    const checkPickGroup = () => {
        try {
            ...
            if (!pickGroupMatch || !sportMatch || (sport !== 'UFC' && sport !== 'MMA'))
                return false;
            const pickGroup = pickGroupMatch[1];
            if (pickGroup === lastSentPickGroup)
                return true;
