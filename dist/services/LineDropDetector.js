...
export class LineDropDetector {
    ...
    static getPlatformSchedule(eventSaturdayMs) {
    ...
    static getPollIntervalMinutes(daysUntil) {
        if (daysUntil > 6.5)
            return null;
        if (daysUntil > 5.5)
            return 60;
    ...
    static async quickCheckUnderdogLines() {
    ...
    // ── DETECT LINE DROPS ──────────────────────────────────────────────────
    static detectDrops(schedule, udCount, prevUDCount, p6Count, prevP6Count, detectedUD, detectedP6) {
        const drops = [];
        // Underdog SS/TD detection (Monday window)
        // Threshold: appeared when wasn't there, or +4 fighters
        if (schedule.find((e) => e.platform === 'underdog') &&
            !detectedUD) {
    ...
    // ── UPDATE POLL RATE ───────────────────────────────────────────────────
    static shouldUpdatePollRate(state, daysUntil) {
    ...
    // ── HELPERS ───────────────────────────────────────────────────────────
    static isOutsideWindow(daysUntil) {
    ...
    static logPollStatus(daysUntil, schedule, udCount, prevUDCount, p6Count, prevP6Count) {
        this.log(`${daysUntil.toFixed(1)}d out | UD:${udCount}(was ${prevUDCount}) P6:${p6Count}(was ${prevP6Count}) | ` +
            `Expecting: ${this.formatSchedule(schedule)}`);
    }
    static logLineDrops(drops) {
        const formatted = drops.map((d) => `${d.platform} ${d.type}(${d.count})`).join(', ');
        this.log(`LINE DROP: ${formatted}`);
    }
    static logWatcherStart(eventName, eventDateStr, daysUntil, pollMins) {
        this.log(`Started — "${eventName}" ${eventDateStr} (${daysUntil.toFixed(1)}d out) | Poll: ${pollMins}min`);
        this.log(`Schedule: Mon=UD/PP SS+TD · Wed=Pick6 FP · Thu-Fri=Betr+PP FP`);
    }
    static logWatcherStop() {
        this.log('Stopped');
    }
}
//# sourceMappingURL=LineDropDetector.js.map