...
function normalizePropType(propType) {
    ...
    if (/^td$/i.test(v))
        return 'TD';
    if (/^fantasy_pp$/i.test(v) || /^fp_pp$/i.test(v))
        return 'Fantasy_PP';
...
function normalizeRecord(record) {
    ...
    if (!fighter || !opponent || !event || !date || !isValidIsoLike(date))
        return null;
    ...
    if (record.platform)
        normalized.platform = String(record.platform).trim().toLowerCase();
    if (record.line != null) {
...
function scoreBackfillCandidate(target, candidate) {
    let score = 0;
    const targetEvent = normalizeEvent(target.event);
    const candEvent = normalizeEvent(candidate.event);
    if (targetEvent && candEvent && targetEvent === candEvent)
        score += 0;
    else if (targetEvent && candEvent && (targetEvent.includes(candEvent) || candEvent.includes(targetEvent)))
        score += 2;
    else
        score += 6;
    const targetOpp = normalizeName(target.opponent);
    const candOpp = normalizeName(candidate.opponent);
    if (targetOpp && candOpp && targetOpp === candOpp)
        score += 0;
    else if (targetOpp && candOpp && (targetOpp.includes(candOpp) || candOpp.includes(targetOpp)))
        score += 2;
    else
        score += 4;
...
export class PropArchiveService {
    ...
    static async getPlatformHistory(fighter, platform, propType) {
        const normalizedFighter = normalizeName(fighter);
    ...
    static async fighterHasFantasyLineHistory(fighter) {
    ...
    static async fighterHasPerformanceHistory(fighter) {
        const rows = await this.getFighterHistory(fighter);
    ...
    static async backfillUnresolvedFromKnownOutcomes(options) {
        ...
        const maxScore = Number.isFinite(options?.maxScore) ? Number(options?.maxScore) : 12;
        const minHoursBetweenRuns = Number.isFinite(options?.minHoursBetweenRuns)
