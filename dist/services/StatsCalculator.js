...
export class StatsCalculator {
    ...
    static calcWinBonus(won, method, round) {
        if (!won)
            return 0;
        const isDec = /DEC/i.test(method || '');
        if (isDec)
            return FANTASY_SCORING.winBonus.decision;
        const r = round || 3;
        if (r === 1)
            return FANTASY_SCORING.winBonus.round1;
        if (r === 2)
            return FANTASY_SCORING.winBonus.round2;
        if (r === 3)
            return FANTASY_SCORING.winBonus.round3;
        return FANTASY_SCORING.winBonus.round4Plus;
    }
    static calcFP(sigStr, totStr, ctrlSecs, kd, td, rev, won, method, round) {
        const nonSigStr = Math.max(0, (totStr || 0) - (sigStr || 0));
        return ((sigStr || 0) * FANTASY_SCORING.sigStrike +
    ...
    static deriveStyle(careerStats) {
        if (!careerStats)
            return 'balanced';
        const { tdAvg, subAvg, slpm } = careerStats;
        if ((tdAvg || 0) > 2 || (subAvg || 0) > 0.5)
            return 'grappler';
        if ((slpm || 0) > 5)
            return 'striker';
        return 'balanced';
    }
    static isFinish(method) {
        return /KO|TKO|SUB/i.test(method || '');
    }
    // ── BUILD FIGHTER DATABASE ────────────────────────────────────────────
    static buildFighterDB(name, ufcData) {
        if (!ufcData) {
            return {
                record: '—',
                country: '🏳️',
        ...
        const avgFP = validFights.length > 0
        ...
        const avgSigStr = fightsSS.length > 0
        ...
        const avgTDperFight = fightsTD.length > 0
        ...
        const oppHistory = history
            .filter((f) => f.oppStats != null)
            .map((f) => {
            const os = f.oppStats;
            const oppWon = f.result === 'loss';
            const fp = os.sigStr != null
                ? this.calcFP(os.sigStr, os.totStr, os.ctrlSecs, os.kd, os.td, os.rev ?? null, oppWon, f.method, f.round)
                : null;
            return {
                opp: f.opp,
    ...
    static parseCareerStats(html) {
        ...
        const liPct = (label) => {
            const re = new RegExp('<i[^>]*>\\s*' + label + ':?\\s*<\\/i>([^<]*?)(\\d+\\.?\\d*)%', 'i');
            const m = html.match(re);
