...
export class PropLinePredictorService {
    ...
    static async getWeights() {
        ...
        if (!merged.ssPaceRenormalizedV13) {
            ...
            if (Number.isFinite(anchor) && anchor > 0 && Math.abs(anchor - 1) > 0.02) {
                ...
                for (const k of Object.keys(merged.ss_pace_modifier)) {
                    const v = merged.ss_pace_modifier[k];
                    if (typeof v === 'number' && Number.isFinite(v))
                        merged.ss_pace_modifier[k] = v * factor;
                }
        ...
        if (!merged.fpModRenormalizedV27) {
            ...
            if (Number.isFinite(anchor) && anchor > 0 && Math.abs(anchor - 1) > 0.02) {
                const factor = 1 / anchor;
                for (const k of Object.keys(merged.fp_global_modifier)) {
                    const v = merged.fp_global_modifier[k];
                    if (typeof v === 'number' && Number.isFinite(v))
                        merged.fp_global_modifier[k] = v * factor;
                }
     ...
     */
    static computeMarketFpShift(archive) {
        const MEASURED_DEFAULT = -11;
        const SHRINK_K = 80;
        const SCORER_FIX_TS = Date.parse('2026-08-16T00:00:00Z');
        const diffs = [];
        for (const r of archive) {
            if (r.propType !== 'Fantasy')
                continue;
            if (!r.platform || !FANTASY_SCORING_BOOKS.has(r.platform))
                continue;
            const line = Number(r.line);
            const res = Number(r.result);
            if (!Number.isFinite(line) || !Number.isFinite(res) || line <= 0)
                continue;
            const ts = Date.parse(r.date);
            if (!Number.isFinite(ts) || ts < SCORER_FIX_TS)
                continue;
            diffs.push(res - line);
        }
    ...
    // ── SS Prediction ───────────────────────────────────────────────────
    static predictSS(fighterDB, opponentDB, scheduledRounds, weights, trend, weightClass, marketFtMin, marketExpectedMin) {
        const reasons = [];
     ...
     */
    static predictSSR1(fighterDB, opponentDB, scheduledRounds, weights, trend, weightClass, marketExpectedMin) {
        const reasons = [];
        const r1Of = (h) => {
            const v = Number(h?.sigStrR1);
            return Number.isFinite(v) && v >= 0 ? v : null;
        };
        const own = (fighterDB.history || []).map(r1Of).filter((v) => v != null);
        ...
        const oppAllowed = (opponentDB?.oppHistory || []).map(r1Of).filter((v) => v != null);
        ...
        38 + Math.min(priorN, 10) * 3.2 + (oppAllowed.length >= 3 ? 6 : 0), 35, 82);
    ...
    static predictFantasy(fighterDB, opponentDB, scheduledRounds, weights, trend, ssLine, tdLine, weightClass, bookPriorFP) {
        ...
        const confidence = clamp(30 + sampleSize * 4 + consistencyBonus + oppBonus + recentBonus, 20, FP_CONFIDENCE_CEILING);
     ...
     */
    static marketLineFor(archiveRecords, fighter, propTypes) {
        ...
        if (opens.length)
            return { line: med(opens), kind: 'open', books: opens.length };
        const closes = rows.map(r => Number(r.line)).filter(v => Number.isFinite(v) && v > 0);
        if (closes.length)
            return { line: med(closes), kind: 'close', books: closes.length };
        return null;
    }
    /**
     ...
     */
    static backtestVsPostedLines(predictionEvents, archiveRecords) {
        const PROPS = [
        ...
        const mkCell = () => ({ n: 0, absSum: 0, sgnSum: 0, mae: 0, bias: 0 });
        const seal = (c) => {
        ...
        for (const p of PROPS) {
            byStat[p.label] = mkCell();
            vsResult[p.label] = mkCell();
        }
        let events = 0;
                ...
                for (const p of PROPS) {
                    ...
                    if (mkt) {
                        ...
                        for (const r of rows) {
                            ...
                            if (!book || !Number.isFinite(bl) || bl <= 0)
                                continue;
        ...
        for (const b of Object.keys(byBook))
            for (const k of Object.keys(byBook[b]))
                seal(byBook[b][k]);
    ...
    static async runLearningCycle(eventName, archiveRecords) {
        ...
        for (const pred of eventPred.predictions) {
            ...
            const targetFor = (stat, types, rawActual, mkt, rlmThreshold) => {
                const posted = this.marketLineFor(archiveRecords, pred.fighter, types);
                if (posted)
                    return { target: posted.line, kind: posted.kind === 'open' ? 'line-open' : 'line-close' };
