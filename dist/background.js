...
function computeFP_PP(stats) {
    ...
    if (stats.won) {
        ...
        if (isFin) {
            if (stats.round === 1)
                fp += s.winBonus.round1;
            else if (stats.round === 2)
                fp += s.winBonus.round2;
            else if (stats.round === 3)
                fp += s.winBonus.round3;
            else
                fp += s.winBonus.round4Plus;
        }
        else {
...
function parsePrizePicksApiFighters(data) {
    ...
    for (const p of projections) {
        ...
        if (stat.includes('significant strike'))
            ...
            lineType = 'td';
        else if (stat.includes('fight time') || stat.includes('fighttime') || stat.includes('fight duration') || stat.includes('rounds'))
            lineType = 'ft';
                                ...
                                func: async () => {
                                    ...
                                    if (preferML) {
                                        ...
                                        for (const el of nameEls) {
                                            ...
                                            if (!oddsEl)
                                                continue;
                                            const oddsText = (oddsEl.innerText || oddsEl.textContent || '').replace(/\s/g, '');
